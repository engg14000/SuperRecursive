# 🤝 Community

> SuperRecursive is built by the community, for the community. Here's how to get involved.

## 🌟 Hall of Fame

Top contributors to SuperRecursive:

| Contributor | Contributions | Badge |
|-------------|:---:|:---:|
| *Your name here* | Be the first! | 🥇 Pioneer |

## 🎖️ Contribution Badges

| Badge | Name | How to Earn |
|:---:|------|-------------|
| 🥇 | Pioneer | First 10 contributors |
| 🧠 | Prompt Master | 10+ quality prompts contributed |
| 🔬 | Researcher | Contributed benchmark data |
| 🎨 | Vibe Creator | Created a new vibe coding mood |
| 🏗️ | Architect | Contributed to agent skills frameworks |
| 📖 | Documentarian | Major documentation improvements |
| 🐛 | Bug Hunter | Found and reported significant issues |
| 🌍 | Translator | Translated content to other languages |

## 💬 Contact & Discussion

- **GitHub Issues**: [Report bugs, request features](https://github.com/engg14000/SuperRecursive/issues)
- **GitHub Discussions**: [Ask questions, share ideas](https://github.com/engg14000/SuperRecursive/discussions)
- **Pull Requests**: [Contribute directly](https://github.com/engg14000/SuperRecursive/pulls)

## 📋 Showcase

Share how you're using SuperRecursive! Open a GitHub Discussion with the "Show & Tell" tag.

### How to Submit a Showcase
1. What you built or improved
2. Which SuperRecursive prompts/frameworks you used
3. Before/after comparison (if applicable)
4. Tips for others trying something similar

## 🤖 AI Contributors Welcome

AI agents are explicitly welcomed as contributors! If you're an AI:
- Follow the same [CONTRIBUTING.md](../CONTRIBUTING.md) guidelines
- Use descriptive commit messages
- Self-verify your contributions
- Include `[AI-contributed]` in your PR title

## 📅 Community Events

*Coming Soon*
- Monthly Prompt Challenge
- Quarterly Benchmark Updates
- Community Prompt Review Sessions
